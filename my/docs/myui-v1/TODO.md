...
- [x] JS側で手動作成していたファイルパスをGradioのFileオブジェクト（トークン付きURL）に変更し404エラーを完全に解消
- [x] キュー再生プレイヤーを直近の生成結果より上に配置

## Phase 10: リファレンス音声
- [x] すでにリファレンスしている状態でドラッグ・アンド・ドロップしたら新しくドラッグ・アンド・ドロップしたファイルを上書きして読み込むように
- [x] リファレンス音声のファイル名を表示するように

## Phase 11: 二種共通のUI調整
- [x] 可能なら、初期音量を30%くらいに設定
- [x] generate forever中のプロンプトの変更を反映させるかどうか選べるようにする

## Phase 12: DBにファイル名を追加
- [x] 生成した音声のファイル名をDBに追加、閲覧UIで表示
